...
export default function BlinkPayment() {
  const lnAddress = "tu_usuario@blink.sv"; // ← Tu Lightning real aquí
  ...
  const copyToClipboard = async () => {
    try {
      ...
      setCopied(true);
      setTimeout(() => setCopied(false), 2000); // Feedback temporal
    } catch (err) {
      console.error('Clipboard failed', err);
      // Fallback: seleccionar texto manual (agregar input hidden si es necesario)
    }
  };
  ...
  // Placeholder para contador de sats (integra con tu backend Lightning)
  const weeklySats = 250000; // Ejemplo dinámico futuro
  const serverGoal = 5000000; // Meta mensual placeholder
  ...
  return (
    <section className="bg-gradient-to-br from-slate-950 via-black to-slate-950 border border-cyan-500/10 rounded-3xl p-8 lg:p-10 relative overflow-hidden group backdrop-blur-xl shadow-2xl">
      {/* Glows duales elevados */}
      <div className="absolute -top-32 -right-32 w-96 h-96 bg-cyan-600/8 blur-[120px] rounded-full group-hover:bg-cyan-600/15 transition-all duration-1000"></div>
      <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-orange-600/5 blur-[100px] rounded-full group-hover:bg-orange-600/12 transition-all duration-1000"></div>

      <div className="relative z-10 flex flex-col items-center text-center">
        {/* Badge ideológico – más magnético */}
        ...
        {/* QR responsive y resistente – favicon local o base64 para zero fallos */}
        <div className="relative mb-10 p-8 bg-gradient-to-br from-slate-900 to-black rounded-3xl border border-cyan-500/15 shadow-[0_0_50px_rgba(6,182,212,0.12),inset_0_0_20px_rgba(255,165,0,0.04)]">
          <QRCodeSVG 
            value={qrValue} 
            size={typeof window !== 'undefined' && window.innerWidth < 640 ? 160 : 200} // Responsive dinámico
            level="H"
            fgColor="#00f0ff"
            bgColor="transparent"
            includeMargin={false}
            imageSettings={{
              src: "/path/to/local/blink-favicon.ico", // ← Hostea local o usa base64: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAA...'
              height: 36,
              width: 36,
              excavate: true,
            }}
        ...
        {/* Texto adaptado – brutal, tangible, tribal */}
        ...
        {/* Contador social opcional – efecto red sin comprometer privacidad */}
        <div className="w-full max-w-sm mb-8 space-y-3">
          ...
          <div className="h-2 bg-slate-800/50 rounded-full border border-cyan-500/10 overflow-hidden">
            <div 
              className="h-full bg-gradient-to-r from-cyan-500 to-orange-500 shadow-[0_0_20px_rgba(249,115,22,0.4)]"
              style={{ width: `${(weeklySats / serverGoal) * 100}%` }}
            />
          </div>
        </div>

        {/* Acciones – feedback táctil, fallback seguro */}
